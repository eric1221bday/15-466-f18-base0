...
for name in to_write:
    ...
    if len(obj.data.vertex_colors) == 0:
        print(
            "WARNING: trying to export vertex color, but object '" + name + "' does not have vertex colors; will output (1.0, 1.0, 1.0)")
    else:
    ...
    for poly in mesh.polygons:
        ...
        for i in range(0, 3):
            ...
            if cols != None:
                col = cols[poly.loop_indices[i]].color
            else:
...
print("Wrote " + str(blob.tell()) + " bytes [== " + str(len(data) + 8) + " bytes of data + " + str(
